...
if __name__ == "__main__":
    ...
    for ds_name_key in ds_name_keys:
        ...
        image_export_path = os.path.join(ds_path_dict[ds_type]['rp'], stage3, 'imgs', ds_name_key)
        ...
        for fold in range(unet1_train_eval_dict['folds']):
            ...
            with torch.no_grad():
                ...
                for b_num, batch in enumerate(test_loader):
                    ...
                    if switch_filter_stage3:
                        stage3_output_one_hot[:, 1] = filter_cap(stage3_output_one_hot[:, 1], mask_filter_cap, convex_hull=True)
                    stage3_output_one_hot = stage3_output_one_hot.to(mopa_dict['dev'])

                    imgnames_5cv[ds_name_key].append(batch_ids)

                    conf_matr = confusion_matrix(stage3_output_one_hot, stage3_targets, num_classes=stage3_num_classes, batch=True,
                                        target_one_hot=target_one_hot)
                    ...
                    if switch_save_images:
                        ...
                        for n in range(stage1_inputs.shape[0]):
                            ...
                            plot_prediction_contour(image=stage1_inputs.cpu()[n][1], groundtruth_seg=stage3_targets.cpu()[n],
                                                    prediction_seg=stage3_output_one_hot.cpu()[n],
                                                    include_classes=ds_dict['ice'][:stage3_num_classes], show_ticks=False, title=title,
    ...
    tabulate_5cv_results(conf_matr_5cv, imgnames_5cv, include_classes, stage3_num_classes, show_violin_plot=True)
"""
    with open(f'run_s3_{ds_name_key}.pickle', 'wb') as fh:
        pickle.dump(conf_matr_5cv, fh)
        pickle.dump(imgnames_5cv, fh)
        pickle.dump(include_classes, fh)
        pickle.dump(stage3_num_classes, fh)
"""
