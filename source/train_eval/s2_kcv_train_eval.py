                ...
                for i, batch in enumerate(train_loader):
                    ...
                    if switch_use_stage1_to_predict:
                        ...
                        if switch_augment:
                            ...
                            stage1_inputs, stage1_prediction = stage1_inputs.cpu(), stage1_output_one_hot.cpu()
                            for x in range(stage1_inputs.shape[0]):
                                stage1_inputs[x], stage1_prediction[x] = train_compose_2(stage1_inputs[x], stage1_prediction[x])
                            stage1_inputs, stage1_prediction = stage1_inputs.to(mopa_dict['dev']), stage1_prediction.to(
                                mopa_dict['dev'])

                            # create the stage2 inputs
                            stage2_inputs = torch.cat((stage1_inputs, stage1_prediction[:,1].unsqueeze(dim=1)), dim=1)
                        else:
                            stage2_inputs = torch.cat((stage1_inputs, stage1_output_one_hot[:, 1].unsqueeze(dim=1)), dim=1)

                        # testplot
                        """
                        inputs_np_0 = stage2_inputs[0].cpu().numpy()
                        inputs_np_img = np.concatenate((inputs_np_0[0],inputs_np_0[1],inputs_np_0[2],inputs_np_0[3]), axis=1)
                        plt.imshow(inputs_np_img, cmap="gray")
                        save_fig_path = os.path.join(ds_path_dict[ds_type]['rp'], stage2, f"{i}_test.png")
                        plt.savefig(save_fig_path)
                        """

                        #print('stage2_inputs', stage2_inputs.shape)
                    else:
                        if switch_augment:
                            stage1_inputs, stage2_targets = stage1_inputs.cpu(), stage2_targets.cpu()
                            for x in range(stage1_inputs.shape[0]):
                                stage1_inputs[x], stage2_targets[x] = train_compose_2(stage1_inputs[x],
                                                                                         stage2_targets[x])
                            stage1_inputs, stage2_targets = stage1_inputs.to(mopa_dict['dev']), stage2_targets.to(
                                mopa_dict['dev'])
                            stage2_inputs = torch.cat((stage1_inputs, stage2_targets[:, 3].unsqueeze(dim=1)), dim=1)
